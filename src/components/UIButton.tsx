...
export default function UIButton({
  ...
  const base =
    "ui-btn relative inline-flex items-center justify-center rounded-2xl px-4 py-2 text-sm font-medium " +
  ...
  const styles = {
    primary:
      "bg-gradient-to-r from-accent-pink/80 to-accent-pink/60 text-white hover:from-accent-pink/90 hover:to-accent-pink/70 shadow-lg shadow-accent-pink/25 hover:shadow-xl hover:shadow-accent-pink/40",
    secondary:
      "bg-brand text-white hover:bg-white hover:text-gray-900 shadow-lg shadow-brand/25 hover:shadow-xl hover:shadow-white/40",
    ghost:
      "bg-white/5 text-neutral-200 hover:bg-white/10 border-white/10 hover:border-white/20 shadow-sm hover:shadow-md",
  } as const;
  ...
  if (asChild) {
    ...
    const childOnMouseMove = child.props?.onMouseMove as React.MouseEventHandler<HTMLElement> | undefined;
    const childOnMouseEnter = child.props?.onMouseEnter as React.MouseEventHandler<HTMLElement> | undefined;
    const childOnMouseLeave = child.props?.onMouseLeave as React.MouseEventHandler<HTMLElement> | undefined;
